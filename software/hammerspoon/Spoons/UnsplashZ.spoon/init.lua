...
local function curl_callback(exitCode, stdOut, stdErr)
    if exitCode == 0 then
        ...
        hs.screen.allScreens()[1]:desktopImageURL("file://" .. localpath)
        hs.screen.allScreens()[2]:desktopImageURL("file://" .. localpath)
    else
        print(stdOut, stdErr)
    end
end
...
local function unsplashRequest()
    ...
    obj.pic_url = hs.execute([[ /usr/bin/curl 'https://source.unsplash.com/user/yanss/likes/3840x2160' |  perl -ne ' print "$1" if /href="([^"]+)"/ ' ]])
